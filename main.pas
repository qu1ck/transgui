...
type
  ...
  TMainForm = class(TBaseForm)
    ...
    procedure txDummy1Click(Sender: TObject);
    procedure txDummyClick(Sender: TObject);
...
procedure TMainForm.txDummy1Click(Sender: TObject);
begin

end;

procedure TMainForm.txDummyClick(Sender: TObject);
begin
  ...
  try
    ...
    for i:=0 to FTorrents.Count - 1 do begin
      ...
      if (LabelFilter <> '') and not VarIsEmpty(FTorrents[idxLabels, i]) then begin
        if not String(FTorrents[idxLabels, i]).Contains(LabelFilter) then
          continue;
      end;
